...
# 修改自 https://github.com/BigBubbleGum/RaspberryBackup/blob/master/rpi-backup.sh

## 以下参数可能需要修改
# 备份文件输出目录
output_path=/mnt/HDD/pi5_backup
# 树莓派的Boot分区路径（可用df -h查看）
dev_boot=/dev/mmcblk0p1
# 树莓派的Root分区路径（可用df -h查看）
dev_root=/dev/mmcblk0p2
...
echo ===================== part 8, compress file =========================
zip $output_path/$img.zip $output_path/$img
rm -rf $output_path/$img

echo "==== All done. img file is under $output_path "
